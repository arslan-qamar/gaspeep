...
export const AlertDetailsScreen: React.FC = () => {
  ...
  const recurrenceLabel =
    alertData?.recurrenceType === 'one_off'
      ? 'One-off notification'
      : 'Recurring notification (daily)';
  const notificationChannels = [
    alertData?.notifyViaPush ? 'Push' : null,
    alertData?.notifyViaEmail ? 'Email' : null,
  ]
    .filter(Boolean)
    .join(' + ') || 'None';
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
            ...
            <div className="p-3 rounded bg-slate-50 dark:bg-slate-900">
              <div className="text-xs text-slate-500 dark:text-slate-400">Notification Type</div>
              <div className="font-semibold text-slate-900 dark:text-white">{recurrenceLabel}</div>
            </div>
          </div>
